...
returns `NA` at every site — the failure mode the `reference_level` bug caused
in 0.2.1, where the error was swallowed by the per-site `tryCatch()` and the
result was indistinguishable from universal convergence failure. The existing
...
So the argument against is not that the change is difficult — it is four
edits, two of them trivial — but that on a design of this size the adjusted
model is unlikely to be estimable site by site. The change is worth making for
callers with larger sample sizes; it should ship with a note in
